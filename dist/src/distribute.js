...
async function distribute() {
    ...
    const network = alephium_config_1.default.networks.devnet;
    console.log(network);
    ...
    const deployments = await cli_1.Deployments.from('./artifacts/.deployments.devnet.json');
    ...
    if (deployed !== undefined) {
        ...
        const waitDistribution = setInterval(async function () {
            ...
            if ((events.length > numEventsClosePool || (state.fields.balance >= state.fields.poolSize && !state.fields.open)) && !awaitForTx) {
                ...
                awaitForTx = true;
                await (0, cli_1.waitTxConfirmed)(nodeProvider, distributionTX.txId, 1, 1000);
